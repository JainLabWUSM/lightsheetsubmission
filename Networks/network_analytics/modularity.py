...
def weighted_network_old(excel_file_path):
    ...
    for _, row in df.iterrows():
        node1, node2 = row["Column A"], row["Column B"]
...
def louvain_mod(G, identifier, directory_path):
...
def louvain_mod_20x(G, identifier, directory_path):
    connected_components = list(nx.connected_components(G))
    total_nodes = float(G.number_of_nodes())
    print(f'nodes: {total_nodes}')
    edge_node_avg = G.number_of_edges()/total_nodes
    print(f"edge/node: {edge_node_avg}")
    running_mod = 0
    for i, component in enumerate(connected_components):
        ...
        # Here you might want to modify edge drawing or other visual elements
        # For example, visualizing with weights or community colors (not shown here)
        
        # Calculate modularity
        modularity = community_louvain.modularity(partition, G.subgraph(component))
        num_nodes = len(component)
        weighted_mod = modularity * num_nodes
        running_mod += weighted_mod
    ...
    modularity = running_mod/(total_nodes)
    ...
    with open(f'{directory_path}/{identifier} network stats.txt', 'a') as f:
        f.write(f'\nlouvain by component weighted modularity: {modularity} constructed from {num_components} components containing {number_of_nodes} nodes')
    f.close()

def louvain_graph(edge_weights, identifier, directory_path):
    G = nx.Graph()
