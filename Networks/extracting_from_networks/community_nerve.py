    ...
    def process_edge(label):
        ...
        for index in indices:
            ...
            if glom_labels[z, y, x]:
                edge_connections.append(glom_labels[z, y, x])
...
glom_name = input("Isolated community Glom mask?: ")
...
print("creating boolean mask for gloms")
glom_bools = gloms == 255
del gloms

print("finding edges that interact w/ gloms")
real_edges = glom_bools * edge_labels_1

del glom_bools

print("Removing edges that don't interact with gloms")
# Flatten the 3D array to a 1D array
...
edge_masks = labels_to_boolean(edge_labels_1, real_edges)

del real_edges

edge_labels_1 = edge_masks * edge_labels_1
 
del edge_masks

print("Binarizing output")
edge_labels_1 = np.where(edge_labels_1 != 0, 255, 0).astype(np.uint8)

print("Saving")
tifffile.imsave("output.tif", edge_labels_1)