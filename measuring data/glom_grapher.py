...
def plot_scatter_unique(x_data, y_data):
    """
    Plot a scatter plot of corresponding data points from two lists.
...
ducts_name = input("Watershed gloms tif?: ")
xy_scale = float(input("xy_scale?: "))
ducts = tifffile.imread(ducts_name)
binary_gloms = binarize(ducts)

while True:
    Q = input("Calculate convex hull? (Y/N): ")
    if Q == 'Y':
        print("Generating convex hull...")
        masks = downsample(binary_gloms, 5)

        masks, _ = flood_fill_hull(masks)
        masks = masks.astype(np.uint8)
        masks = upsample_with_padding(masks, 5, binary_gloms.shape)
        tifffile.imwrite("Convex_Hull.tif", masks)

        break
    elif Q == 'N':
        masks = input('Binary hull?:' )
...
excel_filename = 'y_depth_gloms.xlsx'
...
vals, depth = calculate_y_vals_unique(ducts, masks, xy_scale)
...
# Save the DataFrame to an Excel file
excel_filename = 'y_depth_gloms_unique.xlsx'
df.to_excel(excel_filename, index=False)

print(f'Data saved to {excel_filename}')


plot_scatter_unique(depth, vals)