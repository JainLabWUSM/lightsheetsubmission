...
def cell_glom(labelled_gloms, num_gloms, cells, xy_dims, z_dims):
    ...
    for label in range(num_gloms + 1):
        if label == 0:
            ...
            print(f"Processing glom {label} of {num_gloms}")
...
#Open tiffs to arrays
gloms = input("Seperated gloms?:" )
cells = input("Cellpose cell output?: ")
...
df.to_excel("glom_cells_output.xlsx", index=False)
